  ...
  render() {
    ...
    if (this.props.enabled && this.state.selected) {
      ...
      switch (this.props.daySelectionAnimation.type) {
        case "background":
          dateViewStyle.push({ backgroundColor: "red" });
          break;
        case "border":
          dateViewStyle.push({
      ...
      >
        ...
        >
          {this.props.showDayName && (
            ...
            >
              {this.props.date.format("dd").toUpperCase().substring(0, 1)}
            </Text>
          )}

          {this.props.showDayNumber && (
            <View style={[this.props.selected ? { width: Matrics.CountScale(33), backgroundColor: Colors.APPCOLOR, borderRadius: 20, padding: 5 } : { backgroundColor: 'white' }]}>
              <Text
                style={[
                  { fontSize: Matrics.CountScale(15), fontFamily: Fonts.NunitoSansSemiBold },
                  dateNumberStyle, this.props.selected ? { color: 'white' } : {}
                ]}
                allowFontScaling={this.props.allowDayTextScaling}
              >
