...
class CalendarStrip extends Component {
  ...
  // getNextWeek() {
  //   const nextWeekStartDate = this.state.startingDate.clone().add(1, "d");
  //   if (this.props.onWeekChanged) {
  //     if (this.props.useIsoWeekday) {
  ...
  //Set startingDate to the next week
  getNextWeek() {
    const nextWeekStartDate = this.state.selectedDate.clone().add(1, "d");
    this.setState({ selectedDate: nextWeekStartDate })
    if (this.props.onWeekChanged) {
      if (this.props.useIsoWeekday) {
        this.props.onWeekChanged(nextWeekStartDate.clone().startOf("isoweek"));
      } else {
    ...
    let calendarHeader = this.props.showMonth && (
      <View style={{ flex: 1, flexDirection: 'row', justifyContent: 'center' }}>
        ...
        <WeekSelector
          ...
          weekStartDate={this.state.datesForWeek[1]}
          size={this.state.selectorSize}
        />
        <View style={{ width: Matrics.CountScale(20) }}></View>
        <CalendarHeader
