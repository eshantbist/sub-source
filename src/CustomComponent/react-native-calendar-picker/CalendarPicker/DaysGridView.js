...
export default function DaysGridView(props) {
  ...
  // Get enable week Index.
  let enableWeekIndex = -1;
  if (startFromMonday === true) {
    if (enableWeek === 'Mon') {
      enableWeekIndex = 0;
    } else if (enableWeek === 'Tue') {
      enableWeekIndex = 1;
    } else if (enableWeek === 'Wed') {
      enableWeekIndex = 2;
    } else if (enableWeek === 'Thu') {
      enableWeekIndex = 3;
    } else if (enableWeek === 'Fri') {
      enableWeekIndex = 4;
    } else if (enableWeek === 'Sat') {
      enableWeekIndex = 5;
    } else if (enableWeek === 'Sun') {
      enableWeekIndex = 6;
    }
  } else {
    if (enableWeek === 'Sun') {
      enableWeekIndex = 0;
    } else if (enableWeek === 'Mon') {
      enableWeekIndex = 1;
    } else if (enableWeek === 'Tue') {
      enableWeekIndex = 2;
    } else if (enableWeek === 'Wed') {
      ...
      if (i === 0) {
        ...
        if (dayIndex >= startIndex) {
          if (days.length > 0) {
            ...
            if (dayIndex !== enableWeekIndex && enableWeek !== undefined) {
              disabledDates.push(day);
              dateIsDisabled = true;
            } else {
              dateIsDisabled = false;
            }
            // console.log('dis-->disabledDates-->', disabledDates);
            return (
              <Day
                key={day}
                day={day}
